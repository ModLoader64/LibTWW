...
namespace TWW.API {
    ...
    public class dSv_player_get_bag_item_c  {
        ...
        // #ARRCOUNT 0x6
        private u8[] _unk_0x6() {
            u8[] bytes = new u8[0x6]; for(u32 i = 0; i < 0x6; i++){bytes[i] = Memory.RAM.ReadU8(this.pointer + 0x6 + (i * 1));} return bytes;
        }
        
        private void _unk_0x6(u8[] value) {
            for(u32 i = 0; i < 0x6; i++){Memory.RAM.WriteU8(this.pointer + 0x6 + (i * 1), value[i]);}
        }
    }
}
